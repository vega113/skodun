...
def lifecycle(rec, events=()):
    ...
    if events:
        return {'reason_code': events[0]['cause'], 'attribution': 'audited',
    ...
    return {'reason_code': rec.get('terminal_reason') or None,
            'attribution': 'unattributed', 'cancellation': []}
...
def status_candidates(store, repo, scope, limit):
    ...
    requests = store.list_requests(worktree_root=worktree, limit=1000)
    if scope == 'repository':
        requests = [r for r in requests if r['identity'].get('repo_id') == identity['repo_id']]
    reviews = store.control_reviews(worktree_root=worktree,
        repo_id=identity.get('repo_id') if scope == 'repository' else None, limit=100)
    linked = {link['target_id'] for row in requests for link in row['links'] if link['kind'] == 'review'}
    ...
    rows += [{'kind':'review','id':row['id'],'state':report_state(row),
             ...
             for row in reviews if row['id'] not in linked]
