...
def test_stack_request_adds_attribution_without_changing_full_certification(
    ...
    assert with_stack["lineage_context_diagnostics"]["status"] == "complete"
    assert with_stack["lineage_context_diagnostics"]["candidate_count"] == 0
    assert with_stack["fingerprint_diagnostics"]["exact_state"] == "indexed"
...
def test_checkpoint_lease_includes_provider_admission_wait(monkeypatch):
    d = Defaults(timeout_sec=100, timeout_retries=0, degraded_retries=0)
    monkeypatch.setenv("SKODUN_ADMISSION_WAIT_SECONDS", "17.5")
    expected = pipeline.budget.worst_runtime(d, 2, 0) + 17.5
