...
- `build_dedup_evidence(store, repo, diff: Diff, oid, d: Defaults, dispatch: Dispatch) -> DedupEvidence` — `DedupEvidence = {enabled: bool, candidate_context_hash: str | None}`: when `dispatch.dedup` is false → `{enabled: False, ...}` (the transaction will not suppress); otherwise it packs the candidate's oid context ONCE with the same settings a review would use and records its hash; a packing failure yields `candidate_context_hash=None` (the transaction may then suppress only via the legacy no-context rule, never via context match). ANY exception → `{enabled: True, candidate_context_hash: None}`, logged to stderr — evidence degradation narrows suppression, never widens it.
...
- [ ] **Mutations:** treat `""` as NULL in the rule function (state test fails); widen a packing failure to `enabled=False→True` with a fabricated hash (narrowing test fails).
