...
**Interfaces:**
- Produces, in `base.py`:
  - `ParseResult` (moved from `grok.py`; the six Phase 1 fields stay backward-compatible, plus the new `payload` field below) and the provider-neutral payload helpers `_eligible`/`_valid_payload` **moved here from `grok.py`** (both re-exported from `grok.py` so its tests keep passing unchanged).
  - `UNAVAILABLE_RC = 127`.
