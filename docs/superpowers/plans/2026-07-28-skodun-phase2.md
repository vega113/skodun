...
**Interfaces:**
- `triage --list <id>`: findings with a `refuter` annotation show one extra line: `refuter(<provider>/<model>): <verdict> — <reasoning>` where the displayed reasoning is whitespace-collapsed via the existing `trust.one_line` rule and THEN truncated to 120 chars (reasoning is arbitrary model text; raw newlines would break the one-line-per-item listing). The artifact and any adopted ledger reason keep the original untruncated string.
