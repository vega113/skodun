...
- Modify: `src/skodun/adapters/__init__.py`, `src/skodun/adapters/grok.py` (imports, contract-parametric parse, classify(), resolve_binary(), effort_map())

**Interfaces:**
- Produces, in `base.py`:
  - `ParseResult` (moved from `grok.py`; the six Phase 1 fields stay backward-compatible, plus the new `payload` field below) and the provider-neutral payload helpers moved here from `grok.py`: `_valid_payload` (re-exported from `grok.py` so its tests keep passing unchanged) and the Phase 1 `_eligible` predicate, which is RENAMED `_review_eligible` and lives on `REVIEW_CONTRACT.eligible` — eligibility belongs exclusively to `OutputContract`; if any existing grok test references `_eligible` by name, `grok.py` keeps a one-line alias to `_review_eligible`, otherwise no re-export.
  - `UNAVAILABLE_RC = 127`.
  ...
  - `OutputContract(name: str, json_schema: str, eligible: Callable[[object], bool], validate: Callable[[object], bool])` — the response contract a run is asked for; `eligible` is the envelope-extraction predicate (review: has `summary`/`findings`; refuter: has `verdicts`) so extraction needs no contract-conditionals in adapters. Two instances live here: `REVIEW_CONTRACT` (the Phase 1 review schema + `_valid_payload`) and `REFUTER_CONTRACT` (Task 8's verdicts schema + its validator). Adapters are contract-generic: they pass `contract.json_schema` to their CLI's schema mechanism and validate payloads with `contract.validate`.
  - `class Adapter(Protocol)`: `name: str` (adapter name, e.g. `"codex"`), `provider: str` (e.g. `"openai"`), `stdin_from_prompt_file: bool` (class attr, default `False` — set by adapters whose CLI takes the prompt on stdin; Task 7 honors it),
    ...
    `classify(rc: int, stdout: bytes, stderr: bytes, contract: OutputContract = REVIEW_CONTRACT) -> ClassifyResult` (usable-output precedence needs to know which payload shape counts as usable — a valid refuter response must not be judged by review eligibility),
...
**Files:**
- Create: `tests/adapter_conformance.py`, `tests/fixtures/adapters/xai/{healthy,degraded_stopreason,degraded_stderr,unavailable_auth,healthy_noisy_stderr,refuter_healthy}.txt` + `tests/fixtures/adapters/xai/README` (capture-vs-synthesized provenance per fixture)
...
- Produces: `class AdapterConformance` — a mixin; each adapter's test module subclasses it and supplies `adapter()`, `fixture_dir`, and `effort_reject_case() -> tuple[Reviewer, str] | None` (None = full effort support, must then prove every canonical value maps). The mixin asserts, for the supplied adapter:
  1. `parse(garbage, contract)` for garbage in `{b"", b"{", b"\x00\xff" * 512, b"[]"}` × both contracts → `parse_ok=False`, never raises; `classify(rc, garbage, garbage, contract)` for rc in `{0, 1, 127}` × both contracts never raises;
...
- [ ] **Step 2: Create the SIX xai fixtures** — the complete required set is: `healthy` (archive capture), `degraded_stopreason` (archive capture, `stopReason: Cancelled`), `degraded_stderr` (synthesized, labeled — `tool_error`), `unavailable_auth` (synthesized, labeled — auth-fatal stderr + rc 1 + empty stdout), `healthy_noisy_stderr` (archive-captured healthy stdout paired with synthesized signal-word stderr, labeled), `refuter_healthy` (SYNTHESIZED and labeled — the Phase 1 archive predates the refuter and cannot contain its shape; new adapters capture theirs live in their probe steps).
...
- `_run_reviewer` becomes chain-aware: `_run_chain(head: Reviewer, cfg, d, prompt, cwd, store, scratch: Path, tag: str, contract=REVIEW_CONTRACT) -> _Outcome` — `scratch`/`tag` keep the shipped `_run_reviewer` contract (it creates prompt/stdout/stderr files from them); per-entry, per-attempt filenames extend the tag with the chain ORDINAL, never the reviewer name (names are user input and may contain `/`, `..`, or anything else — config does not constrain them): `f"{tag}.e{i}.a{n}"`; the reviewer name appears only in provenance, and the codex/claude schema sidecars (Task 5) land in the same directory. Chain behavior:
  ...
  3. For each entry: if `store.provider_unavailable_reason(entry.provider, now)` → record a synthetic attempt `{n, provider, model, effort, skipped: "provider marked unavailable: <reason>", classification: {kind: "unavailable", category: "quota", detail: <cached reason>}, rc: null, timed_out: null, duration_sec: null, first_output_sec: null}` (no process ran — every execution field is explicitly null, per step 4b) and continue to the next entry.
  4. Otherwise run attempts within the entry. **Every completed attempt is classified before its output is considered:** `classify(rc, stdout, stderr, contract)` first — `_run_chain` passes its active contract at every classification call — `kind == "unavailable"` → stop this entry immediately (no retries against a dead provider) and advance the chain, after `store.mark_provider_unavailable(provider, reason, category, now + TTL)` **only when `category == "quota"`** (TTL 30 min, `PROVIDER_UNAVAILABLE_TTL_SEC = 1800`; auth/binary/model failures are attempt-local — caching them would let one bad model id black-hole a whole provider); `kind == "degraded"` → consumes the entry's degraded-retry budget exactly like a `ParseResult.degraded` (the two degraded signals are OR-ed — a truncated codex stream must not bypass the retry that a truncated grok envelope gets); only `kind == "ok"` may proceed to `parse` and acceptance. Timeouts keep their own retry budget as in Phase 1.
  4b. **Persisted attempt schema, complete:** every `attempts[]` entry carries `{n, provider, model, effort, rc, timed_out, duration_sec, first_output_sec, classification}` — execution fields (`rc, timed_out, duration_sec, first_output_sec`) are explicitly `null` on synthetic no-process entries (cache skips, missing binary detected without a spawn) — where `classification` is: the attempt's full `ClassifyResult` as `{kind, category, detail}` for every completed (non-timeout) attempt — `ok` and `degraded` included, not just `unavailable`; the synthetic objects shown in step 3 (quota-cache skip) and the rc-127 missing-binary path; and **`null` for a timed-out attempt** (its stdout was truncated to nothing — there is nothing to classify; `timed_out`/duration carry that story). `test_fallback.py` asserts a normal completed attempt's exact `{kind: "ok", category: "", detail: ""}` and a timed-out attempt's exact `classification is None`, alongside the existing binary/quota assertions.
  ...
  6. Chain exhausted (every entry unavailable/skipped) → `_Outcome(None, attempts, "all providers unavailable: <per-entry summary>")` → `failed` record, `trustworthy=false` (existing machinery), banner, exit 4. Never a pass. **Gate interaction, stated precisely:** the failed record does not erase older coverage, with the shipped store's selection rule made explicit: `latest_trustworthy_for` returns the NEWEST trustworthy row for the diff_hash, and THAT row must pass the gate's artifact checks (`base_sha` match included). So identical bytes stay covered when the newest trustworthy row matches the current base; a newer trustworthy row at a different base gates 2 even if an older matching-base row exists (newest-wins is the shipped behavior — fail-closed, kept, now pinned). For content with no prior trustworthy coverage — the acceptance drill's case — the gate answers 2. Pinned in `test_fallback.py`: seeded current-base coverage passes; a rebased-base variant gates 2; and a multiple-trustworthy-rows case (older matching base + newer mismatched base) gates 2.
...
- Every `extra_passes.<name>` object (security and skeptic included — retrofit) gains `{provider, model, effort}` provenance, defined precisely: the ACCEPTED attempt's triplet when one exists; else the TERMINAL attempt's (the last that actually executed); else — nothing ever started a process — explicit `null`s plus a `note` naming the cause, for EVERY no-process-start outcome that can reach persistence: cache-skip, spawn-less missing binary (`resolve_binary()` names a path that does not exist — detected without a spawn), prompt-construction failure, `build_cmd` raising (unsupported effort, schema-sidecar I/O). Tested: an extra pass whose reviewer's binary is missing yields an `extra_passes.<name>` object with explicit null provenance plus a diagnostic note. Adapter-resolution failure is NOT in this list — full-graph resolution happens in preflight, before the lock and before any record exists, so no extra_passes object can exist for it. Per-attempt provenance in `attempts[]` always carries the full chain either way.
...
def test_fallback_chain_recovers(tmp_path, monkeypatch, repo, store):
    ...
    assert first["classification"]["kind"] == "unavailable"   # asserted directly,
    assert first["classification"]["category"] == "binary"    # never inferred from rc
...
def test_provider_state_skips_known_dead_provider_when_quota(tmp_path, repo, store):
    ...
    assert first["classification"] == {"kind": "unavailable", "category": "quota",
                                       "detail": "rate limited"}
    assert "effort" in first
    assert first["rc"] is None and first["timed_out"] is None      # no process ran
...
**Interfaces:**
- `triage --list <id>`: findings with a `refuter` annotation show one extra line: `refuter(<provider>/<model>): <verdict> — <reasoning>` where the displayed reasoning is newline-flattened via the existing `trust.one_line` rule (CR/LF to spaces — that is all it does) and THEN truncated to 120 chars (reasoning is arbitrary model text; raw newlines would break the one-line-per-item listing). The artifact and any adopted ledger reason keep the original untruncated string.
