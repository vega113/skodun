...
- **Generic code / example config split:** repo-layout-specific tables (which path prefixes map to which checklist section, which paths count as tests, which paths trigger the security pass) are **configuration, not code**. Committed code ships generic defaults only — an empty mapping, or a stack-agnostic set of concern words. A concrete stack's tables live in a committed example config under `examples/` (e.g. `examples/scala-angular-monorepo.toml`, named for a stack, never for a company or project) that a user copies into their `<repo>/.skodun.toml`. Parity tests **load the example config** and feed its tables to the code under test, so oracle parity is still asserted end-to-end without an owner-specific default ever entering committed code. The relevant `[defaults]` keys are `checklist_map`, `test_path_patterns`, `security_path_segments`, `security_basename_patterns` (Task 2), and — added during the Task 14 review — `security_prompt_slots`. The last one extends the rule from *tables* to *prompt text*: the security prompt is shipped data, sent to the model in every repo that runs skodun, so the spans of it that name what "risky surfaces" means locally are config-filled slots with generic defaults, and the example config carries the concrete fragments the byte-exact prompt parity test asserts against.
...
- Out of scope for Phase 1 (fail closed where relevant): batched review of oversized diffs (diff over budget ⇒ `diff_truncated=true` ⇒ not trustworthy ⇒ gate exit 2 — never a silent pass); pre-push dispatcher **and with it the dedup probe** (foreground `--now` never dedups in the oracle — every `skodun review` runs a fresh review; the 3-way diff/context dedup probe is dispatcher machinery, Phase 3 — the store nonetheless preserves the `context_hash` NULL-vs-`""` distinction for forward compatibility); same-branch supersede (legacy retires only *prepush*-mode workers, which don't exist in Phase 1); rules-registry authoring/generation/sync (`generate-review-rules.mjs` + `check-review-rules-sync.sh` stay in tubescribes; skodun only *consumes* the generated `docs/review/checklists/*.md` + `code-rules.json`); MCP server; scheduling; non-grok adapters; cloud-bot embed generation; SessionStart delivery hook; macOS notifications; retention/pruning; the legacy `-p` prompt fallback (see Grok binary resolution above); **severity- and confidence-based finding filtering** — the Task 2 `[defaults]` keys `severity_gate` and `confidence_threshold` are declared, validated and bounds-checked so a forward-looking config loads today, but nothing reads either one in Phase 1: `gate.open_findings` returns every untriaged finding whatever its severity, so a single `low` finding is exit 1 even under `severity_gate = "high"`, and the only way to clear a finding is to triage it with an audited reason (documented on `config.Defaults` and pinned by `tests/test_gate.py::test_severity_gate_high_still_blocks_on_a_low_finding`).
...
- **Amended after Task 2 shipped (see Global Constraints → generic code / example config split).** `Defaults` also carries the five owner-specific tables consumed by Tasks 8 and 14 — four repo-layout tables plus, added during the Task 14 review, the security prompt's variable spans. They are already implemented in `src/skodun/config.py` and covered by `tests/test_config.py`; this entry records the schema so plan and committed code agree.
  ...
  | `security_prompt_slots` | `tuple[tuple[str, str], ...]` — `(slot-name, fragment)` pairs filling the security *prompt*'s two variable spans, `surfaces` and `extra_checks`. Fragments may contain `\n` and expand to several prompt lines. Partial tables are fine (an unfilled slot keeps its generic text); an unknown or repeated slot name is a load-time `ValueError` naming the key, so a fragment can never go missing silently | `config.SECURITY_PROMPT_SLOTS` — generic: `surfaces` = `"authentication, authorization, public HTTP endpoints, data access,\nwebhooks, or payments"`, `extra_checks` = `"- webhook validation (shared secrets, signatures, unsigned public ingress)\n- payment and quota integrity (privilege escalation, free usage)"` | T14 |

  TOML shape (arrays; `checklist_map` and `security_prompt_slots` are arrays of two-element arrays so order/first-match-wins and slot naming are explicit):
  ...
  security_prompt_slots = [["surfaces", "auth, public HTTP routes, or payments"]]
  ```
...
**Interfaces:**
- Consumes: `promptbuild` prompt shell, `Reviewer` roles from config, the repo-layout tables `Defaults.security_path_segments` / `Defaults.security_basename_patterns` (when the pass runs), and `Defaults.security_prompt_slots` (what the prompt says "risky surfaces" are).
- Produces:
...
- **This task extends** `examples/scala-angular-monorepo.toml` (created by Task 8) with `security_path_segments`, `security_basename_patterns` and `security_prompt_slots` transcribed from the oracle source, so the oracle's real trigger set *and its exact prompt bytes* stay asserted end-to-end via parity tests that load the example config through `load_config` — while committed code and the shipped prompt stay generic.
...
- `should_run_skeptic(mode: str, trustworthy: bool, findings_total: int) -> bool` — `mode == "now" and trustworthy and findings_total == 0`;
  ...
  `merge_extra_pass(primary: dict, extra: dict, pass_name: str) -> dict` (with the failure path split out as `merge_failed_extra_pass(primary, pass_name, failure_reason)` during the Task 14 review, so no caller demotes a good review by passing the wrong argument) — findings merged with title prefix `(security) ` / `(skeptic) ` (if a title starts with `[rule-id]`, prepend `(extra-pass: <name>) ` to `detail` instead — never pollute `rule_ids` extraction). **Demotion keeps the two axes independent (oracle `merge_extra_pass`, lines 162–273):** a *failed* pass (`merge_failed_extra_pass`, or an `extra` whose `parse_ok` is not exactly `True`) sets the primary's `parse_ok=False` and appends to `failure_reason`; a *degraded* pass sets the primary's `degraded=True` and appends to `degraded_reason` — it does **not** touch `parse_ok`. Either way `trustworthy` recomputes to False on save. A size-capped pass sets `partial_coverage=True` without demotion.
...
- [ ] **Step 3b: Extend `examples/scala-angular-monorepo.toml`** — add `security_path_segments`, `security_basename_patterns` and `security_prompt_slots` with the oracle's exact values to the same `[defaults]` block Task 8 created.
- [ ] **Step 4: Run to verify PASS**
...
**Interfaces:**
- Produces: `import_legacy(store: Store, grok_reviews_dir: Path) -> ImportStats(reviews: int, triage: int, skipped_lines: int, demoted_no_artifact: int, findings_reconciled: int)`.
...
- **A trustworthy import requires the full artifact.** An index row is a derived summary without `findings[]`; storing it as trustworthy would let it satisfy the gate, whose artifact validation (Task 6) then rejects it — stranding the gate at exit 2 forever. So: for each index row that would be trustworthy, load and validate `<id>.json` from the archive; import the full artifact. If the artifact file is missing, corrupt, fails `load_valid_artifact`, or disagrees with the index row on `id` or `diff_hash`, import the row **demoted** (`parse_ok=False`, `failure_reason="legacy import: artifact missing/invalid"`, counted in `demoted_no_artifact`) — history is preserved, trust is not.
- **The findings-count cross-check is ASYMMETRIC.** Demote when the artifact reports *fewer* findings than the index row (or when the row states no usable count at all): an artifact that under-reports could hide open findings, and hidden findings are the only shape here that can produce a false gate PASS. Do **not** demote when the artifact reports *more*: the artifact is what gets imported and what the gate then reads, `load_valid_artifact` has already proved it self-consistent (`findings_total == len(findings)`), and its extra findings can only make the gate stricter (exit 1, findings open) — never more permissive. Those rows import trustworthy and are counted separately in `findings_reconciled`, so an operator can see "imported on the artifact's word" without mistaking it for a demotion.
...
- Known intentional deviation from legacy — **the grok adapter's `degraded` detection is a strict SUPERSET of the oracle's (Task 12)**: three inputs make skodun flag `degraded` where the oracle's `detect_degraded`/`grok_stop_reason` stay silent. All three are one-directional and fail-safe — the worst case is one extra re-review, and none of them can turn a degraded run into a silent all-clear, which is the failure this signal exists to prevent (in the oracle's own corpus, 116 `Cancelled` runs recorded clean with zero findings). Each is pinned by an oracle-gated test that asserts *both* sides — that the oracle really does stay silent and that skodun really does flag — so the rationale cannot go stale while quietly being wrong about the oracle. If one ever fails because the oracle was fixed upstream, delete the note; never loosen skodun to restore the row.
  - **`max turns reached` is matched case-insensitively** where the oracle greps it with `grep -Fq` (case-SENSITIVE, `grok-prepush-review.sh:393`). Everything the oracle flags, plus mixed-case spellings a future grok build might emit. Pinned by `test_oracle_is_case_sensitive_for_max_turns_and_skodun_is_not`.
  - **The root envelope is parsed with `json.JSONDecoder().raw_decode`, not `json.loads`**, so trailing bytes can no longer hide a non-`EndTurn` `stopReason`. The oracle's `grok_stop_reason` uses `json.load`, which raises "Extra data" on ANY trailing byte and is swallowed by its `except Exception`; grok does sometimes emit its final object twice or append a line after it, and the adapter's own payload extractor is deliberately built to survive exactly that. With `loads` at the root the two disagreed *silently and in the dangerous direction*: a good envelope carrying `"stopReason": "Cancelled"` plus one trailing line yielded `parse_ok=True, degraded=False, stop_reason=None` — the exact combination that certifies a review, on a run the model cancelled. Pinned by `test_oracle_misses_stop_reason_after_trailing_data`, plus non-oracle tests for the `EndTurn` (must stay clean), duplicated-object, and leading-whitespace shapes.
  - **The envelope is decoded with `errors="replace"`** where the oracle opens it with a strict `encoding="utf-8"` `json.load`. One invalid byte anywhere in the envelope makes the oracle's decode raise and its `except Exception` swallow the whole stopReason read — and invalid multibyte sequences occur in precisely the truncated runs this signal exists to catch. Pinned by `test_oracle_misses_stop_reason_in_invalid_utf8_envelope`.
- Known intentional deviation from legacy — **the importer's findings-count cross-check is asymmetric, and a stale index summary does not demote its artifact (Task 16)**: the legacy writer appends the `index.jsonl` summary when the first pass finishes, so on a real archive a large share of rows carry `findings_total: 0` beside an artifact holding 2–3 findings that later passes merged in. Under a strict `index == artifact` equality every one of those rows imports demoted; on the real archive that is 130 of 134 demotions, against only 4 genuinely missing artifacts. The check is therefore relaxed in exactly one direction: **artifact reports MORE than the index ⇒ import trustworthy** (counted in `findings_reconciled`), **artifact reports FEWER ⇒ demote**, unchanged. The asymmetry is the point. An artifact that *under*-reports could hide open findings, and a hidden open finding is the only shape here that yields a false gate PASS. An artifact that *over*-reports cannot: the artifact is the record that gets imported and that the gate reads, `load_valid_artifact` has already proved it self-consistent (`findings_total == len(findings)`) before the comparison runs, and its extra findings can only push the gate from 0 to 1 — stricter, never more permissive. The disagreement is a defect of the derived summary, not evidence about the artifact, and demoting on it would discard thousands of already-reviewed findings to guard a direction of error that cannot loosen the gate. Every other demotion trigger is untouched: missing artifact, unreadable/corrupt artifact, a failed `load_valid_artifact`, an `id` mismatch, a `diff_hash` mismatch, and an index row that states no usable `findings_total` at all. Pinned from both sides by `test_artifact_reporting_more_findings_than_the_index_row_is_trusted` and `test_artifact_reporting_fewer_findings_than_the_index_row_is_demoted`, and demonstrated end to end by `test_e2e_artifact_out_reporting_its_index_row_gates_at_one`, which drives the over-reporting case through `run_gate` and asserts exit **1** with the findings open — not 0, and not 2. (Secondary benefit: `demoted_no_artifact` now describes the store's final state. Because index rows are upserted by `id` and the archive re-appends a corrected row for the same `id`, the 130 strict-rule demotions were transient — immediately overwritten by the later agreeing row — so the counter was reporting demotions that no longer existed in the store.)
