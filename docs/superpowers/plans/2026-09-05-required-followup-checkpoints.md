...
The coordinator's independent review reproduced a provider-wait lease mismatch: the chain honored an explicit 1000-second request allowance while checkpoint claims still used the 30-second environment fallback. The claim boundary now uses the same store-matched budget controller, retaining the direct/alien-store fallback. The shipped service regression proves both batch and required security claims remain in flight at the old expiry without extra launches.

The still-valid PR threads were reproduced and fixed together: strict canonical dependency digests; newly skipped candidate invalidation receipts without fake calls; one-based batch reuse identities; live follow-up migration blockers; canonical foreground mode; degraded required read-model eligibility; final publication after annotation. Publication now also verifies the visible required-pass metadata and attempts against the durable payload, so a callback cannot leave a valid old hash attached to changed pass facts.

Validation: the expanded strict pipeline/budget batch passed 279 tests with 65 Python 3.14 unclosed-connection warnings. The final strict continuation/follow-up/queue/read-model/schema/result batch passed 220 tests in 91.12s, followed by 7 publication/retry checks in 6.86s after the last metadata tightening. No protected gate/trust file or process ownership policy changed. The earlier lifecycle sweep result remains historical to its tested commit; current-head CI and the coordinator's final integration checks remain the landing authority.
